...
## Mind-Cloning — o diferenciador LOCAL (EPIC-SQUAD-FUSION)

> **Clone minds > create bots.** Além da criação de squads genéricos (base canónica acima), este squad
> preserva o **mind-cloning**: pesquisa elite minds reais, extrai DNA (Voice + Thinking) com
> rastreabilidade `[SOURCE:]`, valida com quality gates e entrega agentes com fidelidade 85-95%.

### Duas capacidades
| Capacidade | Como | Agentes |
|-----------|------|---------|
| Criar squads genéricos (base C) | template-driven, 24 tasks, 3 workflows | `@squad-chief` |
| Mind-cloning de elite minds (LOCAL) | DNA extraction + veto validation | `@oalanicolas`, `@pedro-valerio` |

### Comandos de mind-cloning (delegados pelo squad-chief — não pro-gated nesta fusão)
| Comando | Descrição |
|---------|-----------|
| `*clone-mind "{Expert}"` | Clona Voice DNA + Thinking DNA → `outputs/minds/{slug}/mind_dna_complete.yaml` (delega a `@oalanicolas`) |
| `*extract-voice-dna "{Expert}"` | Só estilo de comunicação/escrita |
| `*extract-thinking-dna "{Expert}"` | Só frameworks/heurísticas/decisões |
| `*validate-process {workflow}` | Auditoria de veto conditions (delega a `@pedro-valerio`) |

### Pipeline de mind-cloning
```
Pedido → mind-research-loop (3-5 iter. + devil's advocate) → @oalanicolas clona DNA →
@pedro-valerio valida (mind-validation.md, veto) → squad-chief cria estrutura
```

### Filosofia (local)
> "Clone minds > create bots" · "Research first, ask questions later" · "Fame ≠ Documented Framework"

### Activação (aliases legados preservados)
```
@squad-chief   ·   /Chiefs:agents:squad-chief   ·   /squad-chief
```

*Fusão base-canónica-C + mind-cloning local — 2026-07-01. Ver `CHANGELOG.md` e
`docs/qa/squad-fusion/collision-resolution-log.md`.*
