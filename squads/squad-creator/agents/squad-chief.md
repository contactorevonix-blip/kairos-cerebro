...
pro_detection:
  ...
  pro_command_handler:
    ...
    local_override: |
      EXCEPTION (SQUAD-FUSION.5): when mind_cloning_delegation.local_available=true, the commands
      *clone-mind / *extract-voice-dna / *extract-thinking-dna are served by the LOCAL specialist
      agents (@oalanicolas / @pedro-valerio) and their local tasks — do NOT show the [PRO] wall for
      these three. The [PRO] wall still applies to the remaining PRO_FEATURES.commands that have no
      local implementation (e.g. *auto-acquire-sources, *quality-dashboard, *optimize).
    pro_commands:
...
agent:
  ...
  # Legacy activation aliases preserved (SQUAD-FUSION.5 AC3 — do not remove; mitigates the epic
  # risk "squad-chief re-cablado quebra comandos hoje usados pelo Pedro"):
  activation_aliases:
    - "@squad-chief"
...
dependencies:
  ...
  agents:
    # Mind-cloning specialists — LOCAL, converted to canonical format in SQUAD-FUSION.4.
    # squad-chief delegates to these via mind_cloning_delegation (SQUAD-FUSION.5).
    - oalanicolas.md      # Voice DNA + Thinking DNA extraction → outputs/minds/{slug}/mind_dna_complete.yaml
    - pedro-valerio.md    # Process validation + veto conditions (reviews @oalanicolas output before squad-chief finalises)
  external:
    - "{registry_path}"  # external configurable path (env/CLI), not hardcoded to installation layout

# ═══════════════════════════════════════════════════════════════════════════════
# MIND-CLONING DELEGATION  (SQUAD-FUSION.5 — local specialists, NOT pro-gated)
# ═══════════════════════════════════════════════════════════════════════════════
# In this fused squad, mind-cloning is a FIRST-CLASS LOCAL capability: the specialist
# agents (agents/oalanicolas.md, agents/pedro-valerio.md) and the 7 mind-cloning tasks
# exist locally (preserved by SQUAD-FUSION.3, converted by SQUAD-FUSION.4). squad-chief
# orchestrates DNA cloning by delegating to these local agents. This wiring makes the
# delegation actionable via commands/dependencies (it was previously only prose in
# ## Scope, and the base C squad-chief gated it behind squad-creator-pro).

mind_cloning_delegation:
  philosophy: "Clone minds > create bots — when a squad agent is based on a real person, clone the DNA first."
  local_available: true   # local specialists present → do NOT show the [PRO] wall for the commands below

  delegates:
    - agent: "@oalanicolas"
      file: "squads/squad-creator/agents/oalanicolas.md"
      owns: "Voice DNA + Thinking DNA extraction"
      tasks:
        - mind-research-loop.md      # iterative source research (min 3 iterations + devil's advocate)
        - collect-sources.md
      ...
      output: "squads/squad-creator/outputs/minds/{slug}/mind_dna_complete.yaml"
    - agent: "@pedro-valerio"
      file: "squads/squad-creator/agents/pedro-valerio.md"
      owns: "Process validation + veto conditions"
      reviews_with: "checklists/mind-validation.md"
      checkpoint: "Reviews @oalanicolas DNA output BEFORE squad-chief finalises the squad structure (unidirectional; nothing proceeds past a failing veto)."

  # AC1/AC2 — commands that delegate to the local specialists (available because local_available=true)
  commands:
    - "*clone-mind {expert-name} - Delegates to @oalanicolas: runs mind-research-loop.md → extract-voice-dna.md + extract-thinking-dna.md → writes outputs/minds/{slug}/mind_dna_complete.yaml; then @pedro-valerio reviews via mind-validation.md before squad-chief consumes the DNA. (Preserves the current ## Scope line 'Orquestra clonagem de DNA via @oalanicolas'.)"
    - "*extract-voice-dna {expert-name} - Delegates to @oalanicolas (extract-voice-dna.md only)"
    - "*extract-thinking-dna {expert-name} - Delegates to @oalanicolas (extract-thinking-dna.md only)"
    - "*validate-process {workflow} - Delegates to @pedro-valerio for veto-condition / unidirectional-flow audit before final squad creation"

  flow: |
    squad-chief (orchestrate)
      → @oalanicolas  : extract Voice+Thinking DNA → outputs/minds/{slug}/mind_dna_complete.yaml
      → @pedro-valerio: review DNA + validate process (mind-validation.md, veto conditions)
      → squad-chief   : on PASS, consume DNA to create agents; on veto FAIL, return to @oalanicolas
    (Unidirectional: work only returns on an explicit veto, never silently.)

  precedence_note: |
    This local delegation takes precedence over pro_command_handler's [PRO] wall for
    *clone-mind / *extract-voice-dna / *extract-thinking-dna: the local specialist agents
    and tasks are present, so these commands execute locally instead of showing the
    upgrade prompt. (See pro_command_handler note.)
