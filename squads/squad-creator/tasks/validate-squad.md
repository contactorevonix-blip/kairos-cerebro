...
## Mind-Cloning DNA Quality (LOCAL fusion re-attachment — SQUAD-FUSION.6 AC5)

> **Context:** the base C `validate-squad` above validates generic `voice_dna` presence but not the
> **DNA-quality** phases the local B version had (source-traceability of Voice/Thinking DNA). This block
> re-attaches that mind-cloning-specific validation so it is not lost in the fusion. It is an ADDITIONAL
> phase, applied only to squads that use mind-cloning (Expert squads with cloned real-person agents).

### Phase — DNA Quality (mind-cloned agents)
- `outputs/minds/{slug}/mind_dna_complete.yaml` exists and is real content (not placeholder)
- Voice DNA: signature phrases each carry `[SOURCE:]`
- Phrases: 20+ traceable phrases with `[SOURCE:]`
- Thinking DNA / frameworks: heuristics documented with WHEN to use
- Fidelity target: 85-95% (`config.yaml` `mind_cloning.min_fidelity: 85`)

### Task Anatomy (mind-cloning tasks)
For each mind-cloning task (`extract-voice-dna`, `extract-thinking-dna`, `mind-research-loop`, ...):
- [ ] Clear objective
- [ ] Veto conditions defined
- [ ] Output examples
- [ ] Quality gate referenced

**Reviewer:** `@pedro-valerio` via `checklists/mind-validation.md` before squad-chief finalises.
**Dependencies (local):** `checklists/mind-validation.md`, `outputs/minds/`, `core/voice-dna.js`, `core/thinking-dna.js`.
