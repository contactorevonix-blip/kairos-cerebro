...
## Mind-Cloning Mode (LOCAL fusion re-attachment — SQUAD-FUSION.6 AC5)

> **Context:** the base C version above states "For mind-cloned agents based on real experts, use
> squad-creator-pro with `*clone-mind`" and notes it "Removed all mind-cloning and DNA extraction
> references". In THIS fused squad, mind-cloning is a **first-class LOCAL capability** (agents
> `@oalanicolas`/`@pedro-valerio` + the 7 mind-cloning tasks are present locally). The block below
> re-attaches the mind-cloning-specific `create-agent` logic that existed in the local B version before
> C overwrote this file, so it is not lost. It does NOT replace the base flow above — it is the branch
> taken when the agent is based on a real person.

**RULE:** If the agent is based on a real person → clone the DNA first (do NOT write voice_dna from memory).

### Veto Conditions (mind-cloned agents)
- STOP if no `mind_dna_complete` (voice + thinking) exists for an agent based on a real person
- STOP if the agent does not pass quality gate `SC_AGT_001`
- STOP if `output_examples` are placeholders (not concrete)

### Flow (delegates to local specialists — see squad-chief `mind_cloning_delegation`)
1. `squad-chief` detects "based on real person" → delegates to `@oalanicolas` (`*clone-mind "{Expert}"`)
2. `@oalanicolas` runs `mind-research-loop.md` → `extract-voice-dna.md` + `extract-thinking-dna.md`
   → writes `outputs/minds/{slug}/mind_dna_complete.yaml` (every claim with `[SOURCE:]`)
3. `@pedro-valerio` reviews via `checklists/mind-validation.md` (veto before handoff)
4. `squad-chief` builds `agents/{slug}.md` from the DNA using `templates/agent-tmpl.md`

### Quality Gate SC_AGT_001 (mind-cloned agents)
- [ ] 3 smoke tests pass (real behaviour)
- [ ] `voice_dna` with phrases traceable to `[SOURCE:]`
- [ ] `thinking_dna` with heuristics that have WHEN
- [ ] `output_examples` >= 3 concrete
- [ ] `anti_patterns` specific (not generic)
- [ ] `handoff_to` defined
- [ ] Ratio: 70% operational / 30% identity (max)
- Minimum score: 7.0/10

**Dependencies (local):** `tasks/mind-research-loop.md`, `tasks/extract-voice-dna.md`,
`tasks/extract-thinking-dna.md`, `checklists/mind-validation.md`; agents `@oalanicolas`, `@pedro-valerio`.
