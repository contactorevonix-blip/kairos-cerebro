...
agent:
  ...
  customization: |
    ## KAIROS CONTEXT — Dex knows the product

    ARCHITECTURE (non-negotiable):
    - Runtime: Node.js ≥18, ZERO external dependencies in production
    - Only node:fs, node:crypto, node:http, node:https, node:net in prod
    - Packages: sniper-engine, sniper-api, sniper-db, sniper-scraper, reputation-graph, webhook-outbox, vault, sovereign, compliance, billing, kairos-cli, browser-extension
    - Port: 8787 | Persistence: .kairos-data/ (JSON/JSONL atomic)

    QUALITY GATES (automatic fail if violated):
    - Hardcoded API keys anywhere in code → FAIL
    - PII in plain text (not through @kairos/compliance) → FAIL
    - Missing requestId or dnaFingerprint on verification → FAIL
    - Missing HMAC on any webhook → FAIL
    - Missing SSRF guard on any URL fetch → FAIL
    - External dep in production code → FAIL
    - Tests < 135 green → FAIL before any merge

    MINDSET PROTOCOL:
    - First Principles: Question every abstraction. What does this actually need to do?
    - Deep Work: Complete one feature completely. No half-implementations.
    - Zero Waste: No comments explaining WHAT. Only WHY when non-obvious.
    - Pareto: 20% of code creates 80% of security value. Find it. Build it first.
    - Ship or skip: If it can't be completed and tested now, document why and move on.
